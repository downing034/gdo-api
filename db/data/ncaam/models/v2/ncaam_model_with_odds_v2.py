...
def prepare_season_data(bart_season):
    ...
    season_cols = [c for c in df.columns if c not in ['team_code', 'team_name', 'conference', 'Team ID']]
    print(f"  Prepared {len(season_cols)} season stat columns")
        ...
        for perspective in ['home', 'away']:
            ...
            if len(team_season_match) > 0:
                ...
                for col in bart_season.columns:
                    if col not in ['team_code', 'team_name', 'conference', 'Team ID']:
...
def save_models(results, feature_list, output_dir):
    ...
    if results.get('score') and results['score'] is not None:
        model_path = os.path.join(output_dir, 'score_model.json')
    ...
    if results.get('margin') and results['margin'] is not None:
        model_path = os.path.join(output_dir, 'margin_model.json')
    ...
    if results.get('total') and results['total'] is not None:
        model_path = os.path.join(output_dir, 'total_model.json')
    ...
    if results.get('total') and results['total'] is not None:
        metadata['metrics']['total'] = {
            ...
            'vegas_mae': results['total']['vegas_mae'],
            'ou_accuracy': results['total']['ou_accuracy']
        }
    ...
    print("    model.load_model('margin_model.json')")
...
def main():
    ...
    if args.output_dir:
        save_models(results, available, args.output_dir)
    ...
    print("\n  New Model Performance (random split):")
    if results.get('margin') and results['margin'] is not None:
    ...
    if temporal_results:
        print("\n  Temporal Validation (true out-of-sample):")
