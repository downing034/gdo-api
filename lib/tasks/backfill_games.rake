...
namespace :games do
  ...
  desc "Backfill NCAAM games from CSV file (default: yesterday, override with date like ['2025-12-15'])"
  task :backfill_ncaam, [:max_date] => :environment do |t, args|
    max_date = args[:max_date] ? Date.parse(args[:max_date]) : Date.yesterday
    backfill_date_based_league('ncaam', max_date, 'db/data/ncaam_model_tracking.csv')
  end
  ...
  def process_mlb_game_row(row, league)
    ...
    if row['away_result'].present? && row['home_result'].present?
      ...
      GameResult.find_or_create_by(game: game) do |r|
    ...
    # create_game_odds(game, row, league)
  end
  ...
  def backfill_league_games(league_code, max_week, file_path)
    ...
    CSV.foreach(file_path, headers: true) do |row|
      begin
        result = process_week_game_row(row, league, max_week, games_created, games_updated, results_created, games_skipped, odds_created)
        games_created, games_updated, results_created, games_skipped, odds_created = result
      rescue => e
        puts "\nError processing row #{row['id']}: #{e.message}"
        puts "Row data: #{row.to_h}"
        raise e if Rails.env.development?
  ...
  def backfill_date_based_league(league_code, max_date, file_path)
    unless File.exist?(file_path)
      puts "Error: CSV file not found at #{file_path}"
      exit 1
    end
    ...
    league = League.find_by!(code: league_code)
    puts "Processing #{league_code.upcase} games from #{file_path}..."
    puts "Including games up to #{max_date} or games with moneyline odds"
    ...
    CSV.foreach(file_path, headers: true) do |row|
      begin
        result = process_date_game_row(row, league, max_date, games_created, games_updated, results_created, games_skipped, odds_created)
        games_created, games_updated, results_created, games_skipped, odds_created = result
      rescue => e
        puts "\nError processing row #{row['id']}: #{e.message}"
        puts "Row data: #{row.to_h}"
  ...
  def process_week_game_row(row, league, max_week, games_created, games_updated, results_created, games_skipped, odds_created)
    # Parse week first
    week = row['week'].to_i
    
    # Skip games beyond the current week BEFORE parsing other fields
    if week > max_week
      puts "Skipping: Week #{week} (beyond week #{max_week})"
      games_skipped += 1
      return [games_created, games_updated, results_created, games_skipped, odds_created]
    end
    ...
    has_results = row['away_result'].present? && row['home_result'].present?
    initial_status = has_results ? 2 : 0  # final : scheduled
  ...
  def process_date_game_row(row, league, max_date, games_created, games_updated, results_created, games_skipped, odds_created)
    ...
    if game_date > max_date && row['favorite_moneyline_odds'].blank?
      puts "Skipping: #{away_team_code} @ #{home_team_code} on #{game_date} (beyond #{max_date} and no moneyline odds)"
      games_skipped += 1
      return [games_created, games_updated, results_created, games_skipped, odds_created]
    end
